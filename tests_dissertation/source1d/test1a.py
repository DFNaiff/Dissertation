...
def sampling1(nsamples):
    ...
    X2 = logit_t(torch.rand(nsamples,1)*0.4,b=0.4)
    X3 = torch.log(torch.distributions.HalfCauchy(scale=10.0).rsample((nsamples,1)))
    X4 = torch.log(torch.distributions.HalfCauchy(scale=0.1).rsample((nsamples,1)))
    sample = torch.cat([X1,X2,X3,X4],dim=1)
    return sample
