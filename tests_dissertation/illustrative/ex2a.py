...
# and training is made on an almost accurate representation
# =============================================================================
import sys
...
#%%
for jointopt_interval in [1,10,1000]:
    try:
        os.mkdir("ex2a_jopt%i_data"%jointopt_interval)
    except:
        pass
    nsamples = 51
    ninducing = 50
    ...
    for i in range(50):
        ...
        vbp = vb.current_logq(delta_x.reshape(-1,1)).cpu().flatten().numpy().astype(float)
        vbp_list.append(vbp)
        vb.save_distribution("ex2a_jopt%i_data/distrib%i"%(jointopt_interval,i+1))
    ...
    np.savez("ex2a_jopt%i_data/tracking"%jointopt_interval,means=dmeans_np,covs=dcovs_np,
             time=time_np,
